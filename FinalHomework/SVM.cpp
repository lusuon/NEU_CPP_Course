...
#define errorPrecision 1e-2	//误差精度
...
bool SVM::isConvergence()
{
	...
	for (int i = 0; i < sampleNum; i++)
	{
		if (alpha[i] < -errorPrecision || alpha[i] > C + errorPrecision) return false;
		else
		{
			// alpha[i] = 0
			if (fabs(alpha[i]) < errorPrecision && label[i] * gx[i] < 1 - errorPrecision) return false;
			// 0 < alpha[i] < C
			if (alpha[i] > -errorPrecision && alpha[i] < C + errorPrecision && fabs(label[i] * gx[i] - 1)>errorPrecision) return false;
			// alpha[i] = C
			if (fabs(alpha[i] - C) < errorPrecision && label[i] * gx[i] > 1 + errorPrecision) return false;
		}
		sum += alpha[i] * label[i];
	}
	if (fabs(sum) > errorPrecision) return false;
...
void SVM::update(int a1, int a2, double x1, double x2)
{
	...
	if (fabs(alpha[a1]) < errorPrecision || fabs(alpha[a1] - C) < errorPrecision || fabs(alpha[a2]) < errorPrecision || fabs(alpha[a2] - C) < errorPrecision)
		b = (b1_new + b2_new) / 2;
	else
		b = b1_new;
	/*
...
{
	if (a1 < -errorPrecision) return false;
	...
	else//我也不知道为什么这么算，我抄的论文里的,意思是选到超平面距离近的边界
	{
		...
		if (Lobj < Hobj - errorPrecision)
			alpha[a2] = L;
		else if (Lobj > Hobj + errorPrecision)
			alpha[a2] = H;
		else
			alpha[a2] = x2;
	}
	//下降太少，忽略不计
	if (fabs(alpha[a2] - x2) < errorPrecision * (alpha[a2] + x2 + errorPrecision))
	{
		alpha[a2] = x2;
		return false;
	}
	...
	do
	{
		...
		for (int i = 0; i < sampleNum; i++)
		{
			...
			if (Changed)
			{
				...
				for (int j = 0; j < sampleNum; j++)
				{
					if (alpha[j] > errorPrecision && alpha[j] < C - errorPrecision)
					{
						eligSample[cnt++] = j;
					}
				}
			...
			if (alpha[i] > errorPrecision && alpha[i] < C - errorPrecision)
			{
				a1 = i;
				//不满足KKT条件
				if (fabs(label[i] * gx[i] - 1) > errorPrecision)
				{
					//选择第二个变量，优先选下降最多的
					a2 = secondAlpha(i);
		...
		{
			a1 = i;
			if (fabs(alpha[i]) < errorPrecision && label[i] * gx[i] < 1 ||
				fabs(alpha[i] - C) < errorPrecision && label[i] * gx[i] > 1)
			{
				//选择第二个变量,步骤同上
				a2 = secondAlpha(i);
				Changed = takeStep(a1, a2);
